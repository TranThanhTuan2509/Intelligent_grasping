...
# Configure OpenAI-compatible API (supports Gemini, OpenAI, etc.)
api_key = os.environ["OPENAI_API_KEY"]
base_url = os.environ.get("OPENAI_BASE_URL", None)  # e.g. https://generativelanguage.googleapis.com/v1beta/openai/
client = OpenAI(api_key=api_key, base_url=base_url)


def call_llm_with_retry(max_retries=3, base_delay=5, **kwargs):
    """
    Call client.chat.completions.create with retry logic.

    Handles:
    - finish_reason='length' (output truncated): retries with doubled max_tokens
    - API errors (rate limit, timeout, connection, server): retries with exponential backoff
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logging.info(f"[LLM] attempt={attempt+1}, finish_reason={finish_reason!r}, "
                         f"output={str(output)[:120]!r}")

            if isinstance(output, str) and output.strip():
                return output

            last_error = (f"Model returned non-string content "
                          f"(finish_reason={finish_reason!r}): {output!r}")
            logging.warning(f"[LLM] {last_error}")
        ...
        if attempt < max_retries:
            ...
            logging.info(f"[LLM] Retrying in {delay}s (attempt {attempt+2}/{max_retries+1})...")
            time.sleep(delay)
...
langsam_actor = LangSAM(sam_type="vit_b", use_gpu=use_gpu)

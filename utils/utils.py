...
from utils.config import client, langsam_actor, api_key, call_llm_with_retry
...
def process_grasping_result(output, text):
    ...
    # Try fallback formats: "remove obstacle, 5" or "pick object, 3"
    match3 = re.search(r'(?:pick object|remove obstacle)s?,?\s*(\d+)', output, re.IGNORECASE)

    # Extreme fallback: just find the first number that isn't surrounded by words
    match4 = re.search(r'\b(\d+)\b', output)
    ...
    if match1:
        ...
        object_id = int(match3.group(1))
        class_name = text
    elif match4:
        object_id = int(match4.group(1))
        class_name = text
    else:
        class_name = text
